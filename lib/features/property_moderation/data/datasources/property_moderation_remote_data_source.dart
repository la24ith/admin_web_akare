...
import '../models/property_moderation_models.dart';
...
abstract class PropertyModerationRemoteDataSource {
  ...
  Future<void> rejectProperty({required String propertyId, required String reason});
}
...
class PropertyModerationRemoteDataSourceImpl
  ...
  Future<List<PropertyQueueItemModel>> getPendingProperties() async {
    ...
    return (rows as List)
        .map((row) => PropertyQueueItemModel.fromSupabase(row as Map<String, dynamic>))
        .toList();
  }
